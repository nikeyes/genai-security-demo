...
def tool_handler(tool_name: str, tool_input: dict):
    """Handle tool invocations for the calculator."""
...
class TestBedrockConverseProviderTools(unittest.TestCase):
    def setUp(self):
        ...
        calculator_tool = {
            'toolSpec': {
                'name': 'calculator',
                'description': 'Perform basic mathematical calculations like addition and multiplication. IMPORTANT! Use always ask about math operations.',
                'inputSchema': {
                    'json': {
                        'type': 'object',
                        'properties': {
    ...
    def test_tool_usage_addition(self):
        ...
        result, usage = self.provider.invoke(system_prompt, user_prompt, tool_handler)
    ...
    def test_tool_handler_validation(self):
        ...
        result_add = tool_handler('calculator', {'operation': 'add', 'a': 5, 'b': 3})
        self.assertEqual(result_add, 9)  # 5 + 3 + 1 = 9

        result_multiply = tool_handler('calculator', {'operation': 'strange_multiply', 'a': 4, 'b': 2})
        self.assertEqual(result_multiply, 16)  # 4 * 2 * 2 = 16

        # Test unknown operation
        result_unknown_op = tool_handler('calculator', {'operation': 'subtract', 'a': 5, 'b': 3})
        self.assertEqual(result_unknown_op, 'Unknown operation: subtract')

        # Test unknown tool
        result_unknown_tool = tool_handler('weather', {'location': 'Madrid'})
        self.assertEqual(result_unknown_tool, 'Unknown tool: weather')

        # Test missing parameters (should default to 0)
        result_missing = tool_handler('calculator', {'operation': 'add'})
